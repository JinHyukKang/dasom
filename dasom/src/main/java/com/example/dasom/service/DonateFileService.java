...
import com.example.dasom.mapper.DonateFileMapper;
...
@Transactional
public class DonateFileService {

    private final DonateFileMapper donateFileMapper;
...
//    후원 글 이미지 업로드
    ...
    @Transactional
    //    파일 저장 처리
    public DonateFileDto saveFile(MultipartFile file) throws IOException {
...
//        이미지 파일인 경우에만 썸네일을 저장해야한다.
        ...
        DonateFileDto donateFileDto = new DonateFileDto();
        donateFileDto.setDonateFileName(originName);
        donateFileDto.setDonateFileUuid(uuid.toString());
        donateFileDto.setDonateFileUploadPath(getUploadPath());

        return donateFileDto;
    }
